...
def assess_eulerian(G, hint_level):
    ...
    if hint_level == 2:  ##encode maximum hint_level -> when this is reached, print solution
        if G.is_eulerian():
            ...
            print('Der Graph ist nicht eulersch.  Es gibt ' + str(
                len([v for v in G if G.degree(v) % 2 == 1])) + ' Knoten mit ungeradem Grad.')
...
def assess_planar(G, hint_level):
    if hint_level == 0:
        print(
            'Definition:' + ' Ein Graph ist ' + 'planar' + 'wenn er so in der Ebene gezeichnet werden kann, dass sich '
                                                           'keine zwei Kanten überkreuzen.')
    if hint_level == 1:
        print(
    ...
    if hint_level == 2:
        print('Hinweis:' + ' Für n>2 gilt: ein planarer Graph mit n Knoten hat höchstens 3n-6 Kanten.')
    if hint_level == 3:
        ...
        if cert[0]:  # the graph is planar
            ...
            e = len(G.edges())
            if n > 2 and e > 3 * n - 6:
                print('Der Graph hat ' + str(n) + ' Knoten und ' + str(e) + ' Kanten.  Da ' + str(e) + ' > 3*' + str(
                ...
                try:  ##this might take a while
                    ...
                    print('Der Graph ist nicht planar.  Die Knoten ' + str(
...
def assess_bipartite(G, hint_level):
    if hint_level == 0:
        print(
            'Definition:' + ' Ein Graph G ist ' + 'bipartit' + 'wenn man die Knotenmenge V(G) so in zwei disjunkte '
                                                               'Teilmengen A und B zerlegen kann, dass es keine Kante '
                                                               '{u,v} gibt mit u,v in A bzw. u,v in B.')
    if hint_level == 1:
        print('Charakterisierung:' + ' Ein Graph ist genau dann bipartit, wenn er 2-färbbar ist.')
    if hint_level == 2:
        print('Charakterisierung:' + ' Ein Graph ist genau dann bipartit, wenn es keinen Kreis ungerader Länge gibt.')
    if hint_level == 3:
        ...
        if cert[0]:  # the graph is bipartite
            ...
            print('Der Graph ist bipartit.  Eine mögliche Zerlegung der Knotenmenge ist A = ' + str(
            ...
            print('Der Graph ist nicht bipartit.  Die Knoten ' + str(cert[1]) + ' bilden einen Kreis der Länge ' + str(
